...
describe('+ AuthController', () => {
  const mockUser = {
    ...
    password: 'rzx_kugt_no_kaido_wano',
  };
  let token = '';
  ...
  describe('+ GET: /connect', () => {
    ...
    it('+ Fails for a non-existent user', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth('foo@bar.com', 'mabook', { type: 'basic' })
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Fails with a valid email and wrong password', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth(mockUser.email, 'mabook', { type: 'basic' })
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Fails with an invalid email and valid password', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth('maro@swhat.com', mockUser.password, { type: 'basic' })
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
